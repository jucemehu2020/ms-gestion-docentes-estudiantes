...
public class DocenteServiceImpl implements DocenteService {
	...
	public List<DocenteResponseDto> listar() {
		...
		List<DocenteResponseDto> docentesResponseDto = docenteRepository.findAll()
				.stream()
	...
	public DocenteResponseDto actualizar(Long id, DocenteSaveDto docenteSaveDto, BindingResult result) {
		...
		Map<String, String> validacionCamposUnicos = validacionCampoUnicos(obtenerCamposUnicos(docenteSaveDto),
				obtenerCamposUnicos(docenteSaveMapper.toDto(docenteBD)));
		if (!validacionCamposUnicos.isEmpty()) {
			throw new FieldUniqueException(validacionCamposUnicos);
		}
	...
	public EstadoCargaMasivaDto cargarDocentes(MultipartFile file) {
		...
		try (Workbook workBook = new XSSFWorkbook(file.getInputStream())) {
			...
			List<DocenteSaveDto> docentesAcargar = docentes
					.stream()
					.filter(docente -> !docentesExistentes.contains(docente.getPersona())
							&& !docentesEstructuraIncorrecta.contains(docente.getPersona()))
					.toList();
	...
	private DocenteResponseDto crearDocenteReponseDto(Docente docente) {
		List<LineaInvestigacionDto> lineasInvestigacion = lineaInvestigacionMapper
				.toDtoList(docenteLineaInvestigacionRepository.findAllLineasInvByIdDocente(docente.getId()));
	...
	private Map<String, String> validacionCampoUnicos(CamposUnicosDocenteDto camposUnicos,
			CamposUnicosDocenteDto camposUnicosBD) {

		Map<String, Function<CamposUnicosDocenteDto, Boolean>> mapCamposUnicos = new HashMap<>();

		mapCamposUnicos.put("codigo",
				dto -> (camposUnicosBD == null || !dto.getCodigo().equals(camposUnicosBD.getCodigo()))
						&& docenteRepository.existsByCodigo(dto.getCodigo()));
